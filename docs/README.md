...
- [x] 기능 구현을 모두 정상적으로 했더라도 **`요구 사항에 명시된 출력값 형식을 지키지 않을 경우 0점으로 처리`**한다.
- [x] 기능 구현을 완료한 뒤 아래 가이드에 따라 테스트를 실행했을 때 모든 테스트가 성공하는지 확인한다.
- [x] **`테스트가 실패할 경우 0점으로 처리`**되므로, 반드시 확인 후 제출한다.
...
- [x] `클래스를 분리하는 연습` 더 분리해보기
- [x] 필드 수(#)를 줄이기 위해 노력해야함
...
- [x] get으로 꺼내서 사용하지 않고 get을 가진 class에서 작동하고 return만 받게하기
...
- [x] 성공 말고 예외 케이스도 테스트하기
  - [x] 테스트 코드도 리 팩터링하기
  - [x] 테스트 코드에서만 돌아가는 코드 만들지 말기
  - [x] 어려운 테스트는 분리해서 테스트해보기
...
- [x] 고객들이 식당에 방문할 날짜와 메뉴를 미리 선택하면
- [x] 이벤트 플래너가 주문 메뉴,
- [x] 할인 전 총주문 금액,
- [x] 증정 메뉴,
- [x] 혜택 내역,
